...
router.get('/', async function(req, res, next) {
    try {
        ...
        try {
            ...
            provinces = response.data.data.data; // Dữ liệu tỉnh/thành phố
        } catch (error) {
            console.error('Lỗi khi gọi API:', error);
        }

        return res.render('client/checkout', { title: 'Danh sách đơn hàng', provinces, districts });
    } catch (error) {
        console.error("Lỗi khi lấy giỏ hàng:", error);
        return res.render("client/cart", { cart: { items: [] }, message: "Đã xảy ra lỗi khi tải giỏ hàng." });
    }
});
router.get('/getDistricts', async function (req, res, next) {
    try {
        ...
        try {
            ...
            districts = response.data.data.data; // Dữ liệu quận/huyện
        } catch (error) {
            console.error('Lỗi khi gọi API:', error);
        }
...
router.get('/getWards', async function (req, res, next) {
    try {
        ...
        try {
            ...
            wards = response.data.data.data; // Dữ liệu quận/huyện
        } catch (error) {
            console.error('Lỗi khi gọi API:', error);
        }
...
router.get('/vnpay_return', function (req, res, next) {
    ...
    if(secureHash === signed){
        ...
        if(vnp_Params['vnp_ResponseCode'] == '00'){
            ...
            updateStatus(vnp_Params['vnp_TxnRef'], 1);
        }
        else{
